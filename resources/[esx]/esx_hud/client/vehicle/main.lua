...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(PlayerPedId(), false) then
            if IsControlJustReleased(0, 26) then -- Touche C
                isSeatbeltOn = not isSeatbeltOn
                if isSeatbeltOn then
                    TriggerEvent('chat:addMessage', { args = { "Ceinture", "Vous avez mis votre ceinture de sécurité." } })
                else
                    TriggerEvent('chat:addMessage', { args = { "Ceinture", "Vous avez enlevé votre ceinture de sécurité." } })
                end
            end
        end
    end
